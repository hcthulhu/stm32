...
void oled_set_square(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, enum color_t color_in, enum color_t color_out) {
    int8_t x_min = x1 < x2 ? x1 : x2;
    int8_t y_min = y1 < y2 ? y1 : y2;
    int8_t x_max = x1 > x2 ? x1 : x2;
    int8_t y_max = y1 > y2 ? y1 : y2;

    for ( uint8_t x_cur = x_min; x_cur <= x_max; x_cur++ ) {
        for ( uint8_t y_cur = y_min; y_cur <= y_max; y_cur++ ) {
            if ( (x_cur == x_min) || (x_cur == x_max) || (y_cur == y_min) || (y_cur == y_max) ) {
                oled_set_pix(x_cur, y_cur, color_out);
            } else {
                oled_set_pix(x_cur, y_cur, color_in);
            }
        }
    }
}
...
void oled_set_circle(int32_t x, int32_t y, int32_t r, int32_t d, enum color_t color_in, enum color_t color_out) {
    ...
    for ( int32_t x_cur = x; x_cur <= x_mid; x_cur++ ) {
        for ( int32_t y_cur = y; y_cur <= y_mid; y_cur++ ) {
            l = (x_cur - x_mid)*(x_cur - x_mid) + (y_cur - y_mid)*(y_cur - y_mid);

            if ( (l <= r*r) && (l >= (r - d)*(r - d)) ) {
                oled_set_pix(x_cur, y_cur, color_in);
                oled_set_pix(x_cur, 2*y_mid - y_cur, color_in);
                oled_set_pix(2*x_mid - x_cur, 2*y_mid - y_cur, color_in);
                oled_set_pix(2*x_mid - x_cur, y_cur, color_in);
            } else if ( l < (r - d)*(r - d) ) {
                oled_set_pix(x_cur, y_cur, color_out);
                oled_set_pix(x_cur, 2*y_mid - y_cur, color_out);
                oled_set_pix(2*x_mid - x_cur, 2*y_mid - y_cur, color_out);
                oled_set_pix(2*x_mid - x_cur, y_cur, color_out);
            }
        }
    }
}
...
void oled_set_model(const pxl model[], uint8_t x, uint8_t y, enum color_t color) {
    for (int i = 0 ; (model[i].x != last_pxl.x) && (model[i].y != last_pxl.y); i++) {
        oled_set_pix(x + model[i].x, y + model[i].y, color);
    }
}
